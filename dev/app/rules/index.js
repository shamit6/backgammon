...
const getPoint = (board, pointId) =>
  board.find((point) => point.pointId == pointId);

const isPointFree = (point) =>
  point.isClient || point.amount <= 1 || point.pointId == 25;

const numberOfCheckersInBoard = (
  board,
  isClient,
  minPoint = 0,
  maxPoint = 25
  ...
  board.reduce((sumChecker, point) => {
    if (
      point.isClient == isClient &&
      point.pointId >= minPoint &&
      point.pointId <= maxPoint
    ) {
...
const isStuckInPoint = (pointId, board, steps, clientState) => {
  const possibleTargetPointsIds = canBeDraggedTo(
    pointId,
    board,
    steps,
    clientState
  ...
  return (
    board.findIndex(
      (p) => possibleTargetPointsIds.indexOf(p.pointId) > -1 && isPointFree(p)
    ) == -1
  );
};
  ...
  if (singleSteps.length > 2) {
    ...
    if (typeof secondStep !== "undefined") {
      ...
      if (isFirstStepFree || isSecondStepFree) {
        const combinedPoint = getPoint(
          board,
          Math.min(pointId + firstStep + secondStep, 25)
        );
...
const isStuckInBoard = (board, steps, clientState) =>
  board
    .filter((p) => p.isClient && p.amount > 0)
...
const isPointCanDragTarget = (pointId, clientStatus) => {
  ...
  if (
    clientStatus == IN_GAME_STATUS.STUCK ||
    (clientStatus == IN_GAME_STATUS.EATEN && pointId > 6)
  ) {
    return false;
  }
...
const canBeDraggedTo = (pointId, board, singleSteps, clientState) => {
  if (clientState == IN_GAME_STATUS.EATEN) {
    if (pointId == 0) {
      return singleSteps
        .map((step) => pointId + step)
        .filter((pointId) => isPointFree(getPoint(board, pointId)));
    } else return [];
  }

  const continiousSteps = possibleClientContiniousSteps(
    pointId,
    board,
    singleSteps
  );
  ...
  if (clientState == IN_GAME_STATUS.DROPOUT) {
    if (Math.min(...singleSteps) > 25 - pointId) {
      const farestPoint = board
        .filter((point) => point.isClient && point.amount > 0)
        .sort()[0];
