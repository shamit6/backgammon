...
const initSocket = (store, username) => {
  ...
  socket.init(document.location.host, {
    query: `username=${username}&isGuest=${isGuest}`,
  });
...
const socketIoMiddleware = (store) => (next) => (action) => {
  if (action) {
    ...
    if (
      (action.type == actions.STEP ||
        action.type == actions.SWITCH_TURN ||
        action.type == actions.DICING) &&
      !action.fromServer
    ) {
  ...
  if (action) {
    ...
    if (
      action.type == actions.STEP &&
      store.getState().app.game.clientStatus == IN_GAME_STATUS.WINNER
    ) {
