...
const updateSteps = (fromPoint, toPoint, currentStep) => {
  ...
  if (stepIndex > -1) {
    updatedSteps = [
      ...currentStep.slice(0, stepIndex),
      ...currentStep.slice(stepIndex + 1),
    ];
...
const moveChecker = (fromPoint, toPoint, isClientTurn, checkersState) => {
  ...
  const sroucePoint = newCheckersState.find(
    (point) => point.pointId == fromPoint
  );
  const tragetPoint = newCheckersState.find(
    (point) => point.pointId == toPoint
  );
  ...
  if (
    isClientTurn &&
    !tragetPoint.isClient &&
    tragetPoint.amount == 1 &&
    tragetPoint.pointId != 25
  ) {
    //ignore eating in dropout
    ...
    !(isClientTurn && tragetPoint.pointId == 25) &&
...
const reducer = (state = getInitialGameState(), action) => {
  switch (action.type) {
    case DICING: {
      ...
      const clientStatus = state.clientTurn
        ? getStateByBoard(state.checkersState, newSteps)
        : IN_GAME_STATUS.ONGOING;
    ...
    case STEP: {
      ...
      const newCheckersState = moveChecker(
        fromPoint,
        toPoint,
        isClient,
        state.checkersState
      ...
      const playStatus =
        clientStatus == IN_GAME_STATUS.WINNER ||
        clientStatus == IN_GAME_STATUS.LOSER
    ...
    case SRART_GAME: {
      return {
        ...getInitialGameState(),
        clientTurn: action.content.isClientStart,
        playStatus: PLAY_STATUS.PLAY,
        opponentInfo: action.content.opponentInfo,
      };
    }
