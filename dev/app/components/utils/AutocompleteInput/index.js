...
const SearchItem = ({itemData, onItemSelect, renderItem, addedClassName, onMouseEnter}) =>
                        <div className={classNames(style.searchItem, addedClassName)}
...
class AutocompleteInput extends Component {
  ...
  componentDidMount(){
    ...
    if (defaultValue){
      this.props.searchPromise(defaultValue)
      .then(res => {
  ...
  onItemSelect(item) {
    return () => {
      this.setState({searchResults:[], serachValue:item[this.props.searchableKey]})
  ...
  handleSearchChange(e, {value}) {
    ...
    if (value.length < 2){
      this.setState({ isLoading: false, searchResults:[], serachValue: value });
    }else{
  ...
  fetchData(value){
    this.props.searchPromise(value)
      .then(res => {
    ...
    const {serachValue, focusedItem, searchResults} = this.state;
      ...
      if (e.keyCode == 13){
        ...
        if (e.keyCode == 40){
          ...
          const nextFocused = (focusedItem + 2) % (searchResults.length+1) - 1;
          this.setState({focusedItem:nextFocused});
        }
        // up
        if (e.keyCode == 38){
          const nextFocused = (focusedItem>=0)?(focusedItem - 1):(searchResults.length-1);
          this.setState({focusedItem:nextFocused});
        }
      }
  }
  ...
  render() {
    ...
    const { serachValue, searchResults, isLoading, focusedItem, showResults } = this.state;

    return <div ref={e => this.elementRef = e}
                ...
                onBlur={() => {
                    ...
                    onFocus={::this.onInputFocus}
  						...
  						{showResults && searchResults.map((item, index) =>
  											<SearchItem addedClassName={(focusedItem==index)?style.focusedSearchItem:null}
                                    key={index}
                                    refkey={index}
  																	itemData={item}
