...
class PlayerStatsViewer extends Component {
  ...
  render(){
    const {userInfo: user, playerStat, playerGames, loadMoreEntries} = this.props;
    let winsLossesChart;
    if (playerStat !== undefined){
      console.log(window.chartColors);
        //TODO
        const data = {
          	labels: Object.keys(playerStat.record).filter(key => key != "__typename"),
          	datasets: [{
          		data: Object.values(playerStat.record).filter(value => value != "PlayerRecord"),
          		backgroundColor: CHART_COLORS,
          		hoverBackgroundColor: CHART_COLORS
          	}]
          };

        winsLossesChart = <PieChart data={data}
            options= {{
    ...
    return <div className={style.playerStats}>
                ...
                <DynamicLoadTable
                  ...
                  fetchingMedthod={loadMoreEntries(() => this.setState({moreItemToLoad:false}))}
                  headerRow={headerRow}
...
const PlayerStatsViewerWithData = graphql(playerStatsQuery, {
  ...
  props: ({ownProps,  data: { loading, playerStat, playerGames, fetchMore }}) => ({
    ...ownProps,
    loading,
    playerStat,
    playerGames,
    loadMoreEntries: (moreItemToLoad) => () =>
     fetchMore({
        ...
        updateQuery: (previousResult, { fetchMoreResult }) => {
          ...
          return {...previousResult,playerGames: [...previousResult.playerGames, ...fetchMoreResult.playerGames] };
        }
      })
  })
})(PlayerStatsViewer);
